    ...
    for m=1:length(constrained_modes)
        if constrained_modes(m)
            ...
            elseif strcmp(constraints{m}{1},'unimodality')
                nn = constraints{m}{2};
                prox_operators{m} = @(x,rho) cmtf.project.project_unimodal(x,nn); % unimodality column-wise
            ...
            elseif strcmp(constraints{m}{1},'non-negative l2-sphere')
                eta = constraints{m}{2};
                prox_operators{m} = @(x,rho) cmtf.prox.prox_normalized_nonneg(x); %(hard) l2 normlization column-wise (||x||_2<=eta) and non-negativity (not convex!)
            ...
            elseif strcmp(constraints{m}{1},'quadratic regularization')
             ...
             elseif strcmp(constraints{m}{1},'TV regularization')
                eta = constraints{m}{2};
                prox_operators{m} =  @(x,rho) cmtf.prox.prox_TV(x,eta/rho); 
             ...
             elseif strcmp(constraints{m}{1},'tPARAFAC2') %tPARAFAC2 constraint, can only be imposed on the second mode of PARAFAC2 (Bk's)
                eta = constraints{m}{2};
                prox_operators{m} = @(x,rho) cmtf.prox.t_smoothness_prox(x,rho,eta); 
                reg_func{m} =  @(x) cmtf.losses.t_smoothness_penalty(x,eta);
