% Test coupling type 4 (C = Delta*H) with KL divergence loss.
%
% Same coupling geometry as test_script_coupling_type4 but with KL loss.
% Expected to fail until ADMM_coupled_case4 lbfgsb_update call is fixed.

function tests = test_KL_coupling_type4
...
function test_workflow(testCase)
    ...
    coupling.coupl_trafo_matrices{4}(1, 1) = 1;
    coupling.coupl_trafo_matrices{4}(2, 2) = 1;
    coupling.coupl_trafo_matrices{4}(5, 3) = 1;
