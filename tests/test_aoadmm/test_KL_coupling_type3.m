% Test coupling type 3 (C = H*Delta) with KL divergence loss.
%
% Same coupling geometry as test_script_coupling_type3 but with KL loss.
% Expected to fail until ADMM_coupled_case3 lbfgsb_update call is fixed.

function tests = test_KL_coupling_type3
...
function test_workflow(testCase)
    ...
    coupling.coupl_trafo_matrices{4}(1:10, 1:10)   = eye(10);
    coupling.coupl_trafo_matrices{4}(11:15, 21:25) = eye(5);
