...
function test_workflow(testCase)
    ...
    % create data
    [X, Atrue, Deltatrue,sigmatrue] = cmtf.utils.create_coupled_data_unimodalBks('model', model, 'size', sz, 'modes', modes, 'lambdas', lambdas_data, 'noise', noise,'coupling',coupling,'normalize_columns',normalize_columns,'distr_data',distr_data,'loss_function',Z.loss_function); %create data
    ...
    testCase.verifyEqual(out.f_tensors, 0.051502, "AbsTol", 1e-3);
    testCase.verifyEqual(Fit1, 94.85, "AbsTol", 1e-3);
    testCase.verifyEqual(FMS_A, 0.9968, "AbsTol", 1e-3);
    testCase.verifyEqual(FMS_B, 0.9685, "AbsTol", 1e-3);
    testCase.verifyEqual(FMS_C, 0.9983, "AbsTol", 1e-3);
    testCase.verifyEqual(out.OuterIterations, 190);
end
