% Test cmtf_AOADMM with beta divergence loss (beta != 0, 1).
%
% Single CP tensor of order 3, size [20,15,12], 2 components, beta=1.5.
% Data is Gamma-distributed (positive), matching the beta-divergence assumption.
% Covers cmtf_AOADMM.m lines 99-105 (beta loss setup).

function tests = test_beta_divergence
...
function test_workflow(testCase)
    ...
    loss_function{1}       = 'beta';
    loss_function_param{1} = 1.5; % beta != 0 and != 1
